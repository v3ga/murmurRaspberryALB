...
void testApp::update()
{
    ...
    if (mp_deviceEcho)
	{
		if (mp_deviceEcho->m_resetVolHistoryPing)
		{
            OFAPPLOG->println("RECEIVED A PING !");
		...
		if (mp_deviceEcho->mp_soundInput &&  mp_deviceEcho->mp_soundInput->getVolHistoryLast() >= mp_deviceEcho->m_volHistoryPingTh && m_canRestartTimeline/* && m_isTimelineStarted == false*/)
		{
			OFAPPLOG->println("TIMELINE STARTS!");
			m_timeline.waitForThread(true);
			m_timeline.start();
			m_isTimelineStarted = true;

			m_canRestartTimeline = false;
			m_timeRestartTimeline = 0.0f;
		}
